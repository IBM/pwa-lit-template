...
import { GetUsers, GetUsersVariables } from '../@types/__generated__/GetUsers';
...
export class PageUsers extends ApolloQuery<GetUsers, GetUsersVariables> {
  query = GET_USERS;
  ...
  private renderUsers() {
    ...
    return html`
      <ul>
        ${users.map((user) => html` <li>${user!.username}</li> `)}
      </ul>
